...
def download_scene_image(prompt: str, idx: int) -> str:
    """
    Download cinematic AI image from Pollinations.ai.
    Completely free, no API key.
    seed varies per scene for unique images.
    """
    full_prompt = (
    ...
    print(f"[IMG] Scene {idx+1}: Generating...")
    try:
        r = requests.get(url, timeout=60)
        r.raise_for_status()
        with open(path, "wb") as f:
            f.write(r.content)
        print(f"[IMG] ✅ scene_{idx:02d}.jpg")
    except Exception as e:
        print(f"[IMG] ⚠️ Pollinations failed ({e}) — black frame fallback")
        subprocess.run([
            "ffmpeg", "-y", "-f", "lavfi",
            f"-i", f"color=c=black:s={VIDEO_W}x{VIDEO_H}:d=1",
            "-frames:v", "1", path
        ], capture_output=True)
...
def build_final_video(
    ...
    # ── SRT caption style ─────────────────────
    abs_srt = str(Path(srt_path).resolve()).replace("\\", "/")
    # Windows path fix
    if ":" in abs_srt:
